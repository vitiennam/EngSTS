...
function changeWord(this: GlobalEventHandlers, evt: MouseEvent){
  // let targetTag = evt.target as HTMLButtonElement | null
  index += 1
  document.getElementById("nextword").innerHTML ='Click '+ randomArray[index]
  userHistory.push(randomArray[index])
  document.getElementById('iframeContent').setAttribute('src', "https://dictionary.cambridge.org/dictionary/english-vietnamese/" + randomArray[index])
  if(index == randomArray.length - 1)
  {
...
document.onreadystatechange = () => {
  ...
  document.getElementById("nextword").onclick = changeWord
